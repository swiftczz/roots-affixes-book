...
> 日常词最会伪装:你每天见它几十次,它却从不主动交代自己一千年前叫什么。今天请 `woman`、`silly`、`nice` 和 `bead` /bid/ 依次接受词源问询。
...
古英语 *gebed* 意为"**祈祷**"(今天的 `bid` 命令、`bede` 祈祷还保留着)。中世纪,基督徒用一串珠子来**数祈祷次数**(念珠,rosary /ˈroʊzəri/)。
...
| `kingly` /ˈkɪŋli/ | `royal` / `regal` /ˈriɡəl/ |
| `hearty` /ˈhɑrti/ | `cordial` |
| `freedom` | `liberty` |
| `help` | `aid` |
