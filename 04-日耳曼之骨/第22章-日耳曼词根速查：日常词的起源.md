...
1. **(破除误解)** `silly` 曾是"幸福的",`nice` 曾是"无知的",今天却没人这么用。有人主张"一个词的词源义/本义才是它真正正确的意思"。用 silly、nice 说明这种主张(所谓"词源谬误")错在哪;那词源的用处到底是什么?
2. **(讲证据)** `heart` / `cordial` / `cardiac` 是三兄弟,分别来自哪条语言路线?你能从词形上用什么线索判断某个"心"词走的是本族、拉丁还是希腊路?
3. **(迁移应用)** `woman`(wīf+man)、`lord`(hlāf-weard)、`goodbye`(God be with ye)都是"磨损复合词",原零件已看不出来。请还原 `daisy` 和 `midwife` 的本义;并说说:这类词提醒你,拿现代拼写现场拆词(如 wo+man)有什么风险?
