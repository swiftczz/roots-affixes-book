...
> **提示** **希腊三尺度**:`micro-` 可表示“小”,在 SI 前缀中又专门表示百万分之一;`macro-` 表示宏观或大尺度;`mega-` 在日常构词中可表示巨大,在 SI 中专门表示一百万。`micrometre` 是 SI 单位表达,`megabyte` 借用了 `mega-` 的数量级习惯,但 byte 本身不是 SI 单位——别把电脑硬塞进国际单位制宿舍。
...
前面的五个抽屉已经把二十五个组合形式摆过一遍,这里不再让它们换成方框重新列队。收银前只扫货架号:

- 自然界:`bio-、geo-、hydro-、photo-、thermo-、aero-`
- 大小多少:`micro-、macro-、mega-、poly-`
- 时空运动:`tele-、archa-、kaino-、neo-、chrono-`
- 同异自他:`auto-、homo-、hetero-、pan-`
- 身体生命:`cardio-、neuro-、derm-、path-、hema-/hemo-、ophthalmo-`

需要按字母查词时,直接去[附录 B](../07-附录/附录B-词根总索引.md);本章负责讲抽屉,索引负责找螺丝,两边不用抢同一份工作。
...
1. `homosexual`(同性恋)、`homogeneous`(同质的)里的 `homo-`,和 `Homo sapiens`(智人)里的 `Homo`,拼写一模一样——是同一个词根吗?你怎么避免这类"同形不同源"的坑?
2. `-phobia` 来自希腊普通名词 *phobos*(恐惧),神话里也有恐惧的拟人 Phobos。到底是"phobia 来自神名 Phobos",还是反过来?你依据什么判断?这和第 17 章的 morphine/morphology 是不是同一个问题?
3. `television` = tele-(希腊"远")+ vision(拉丁"看"),是希腊拉丁混血词。为什么 19 世纪以后这种混血词能被接受?再判断 `automobile`(auto- 希腊"自" + mobile 拉丁"动")是不是也是混血——这类词能通用,说明造词的标准到底是什么?
