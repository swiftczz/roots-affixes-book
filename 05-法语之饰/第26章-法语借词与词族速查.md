...
本章表格里的“来源”栏采用压缩写法:写“法语 X ← 拉丁 Y”时同时给出英语的近站和更早祖籍;若只写拉丁祖形,表示在提示词族亲缘,**不自动等于英语从拉丁直接借入**。少数词还会作为对照进入表中,并不宣称整张表每个成员都由诺曼人同一批送货。
...
| `master` /ˈmæstər/ | 古英语时期已从拉丁 *magister* 一系借入,后来又受法语影响 | 主人、硕士;不能算诺曼征服才第一次进门 |
...
跟“吃”和“打仗”一样,艺术、珠宝、奢华与快活领域也吸收了许多法语词。别因此给中世纪农民发一张“禁止快乐证”:他们当然会唱歌、喝酒、打扮和庆祝。这里反映的是借词渠道、书面记录与社会语域,不是哪个阶层垄断了享受这门手艺。
...
| `-or/-our`、`-er/-re` 等拼写差异 | color/colour, center/centre;主要提示英美拼写史,不能单独证明词源 |
| ch 读 /tʃ/ | 可能是法语词,也可能是本族词 |
| 词首 gu- | guard, guarantee 等部分词可提示法语路线;`guess` 等需逐词另查 |
...
1. `parliament` 为什么更准确地解释为"谈话、讨论、会议",而不是"`-ment` 表地点"?这里的 `-ment` 到底起什么作用?(可对照第 9 章)
2. `soldier` 与 `salary` /ˈsæləri/ 的证据强度有什么不同?为什么可以比较肯定地说 soldier 与 *solidus*(金币)有关,却不能把 salary 直接译成"盐钱"?
3. 本章说 `governor`(州长)和 `cyberpunk`(赛博朋克)是远房亲戚,都来自希腊 `kybernan`(掌舵)。`cyber-` 系列听着最"现代科技",怎么会和古老的 government 同源?你会用什么办法确认这种"八竿子打不着"的同源关系?
...
*下一卷 → [第 27 章 否定前缀为什么这么多：un-, in-, dis-, a-](../06-词缀的故事/第27章-否定前缀为什么这么多：un-,%20in-,%20dis-,%20a-.md)*
