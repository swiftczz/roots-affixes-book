...
1. **(破除误解)** `parliament` 为什么更准确地解释为"谈话、讨论、会议",而不是"`-ment` 表地点"?这里的 `-ment` 到底起什么作用?(可对照第 9 章)
2. **(讲证据)** `soldier` 与 `salary` /ˈsæləri/ 的证据强度有什么不同?为什么可以比较肯定地说 soldier 与 *solidus*(金币)有关,却不能把 salary 直接译成"盐钱"?
3. **(迁移应用)** 本章说 `governor`(州长)和 `cyberpunk`(赛博朋克)是远房亲戚,都来自希腊 `kybernan`(掌舵)。`cyber-` 系列听着最"现代科技",怎么会和古老的 government 同源?你会用什么办法确认这种"八竿子打不着"的同源关系?
