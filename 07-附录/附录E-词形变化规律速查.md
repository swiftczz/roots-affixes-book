...
希腊词根拼接时,常用 `-o-` 作连接元音:bi(o) + logy → biology,therm(o) + meter → thermometer。拉丁词根拼接则多用 `-i-`:carn**i**vore(食肉)、agr**i**culture(农业)、magn**i**fy(放大)。
...
```mermaid
flowchart TD
    S1["① 剥前缀<br/>注意同化还原:attract → ad + tract"]
    S2["② 认词干<br/>想两个干与弱化:cept → cap"]
    S3["③ 剥后缀<br/>-ion / -able / -ism……"]
    S4["④ 组合假设<br/>用核心义推一个大致方向"]
    S5["⑤ 查词典核对<br/>假设不是结论"]
    S1 --> S2 --> S3 --> S4 --> S5
```
