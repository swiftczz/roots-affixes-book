...
stare 家族是**英语里派生能力很强**的拉丁词根家族之一。具体数量会随词源口径而变化,但常见成员已经排成一条长队:`state`(国家)、`station`(车站)、`status`(地位)、`stable`(稳定的)、`institution`(机构)、`constitution`(宪法)、`substance`(物质)、`exist`(存在)、`resist`(抵抗)……
...
黄昏时分,罗马广场中央立着一尊铜像——某位早已作古的执政官,大理石的脚踩在底座上,一立就是几百年。罗马人管这种"立着的东西"叫 ***statua***(英语 `statue` 雕像);管它立在那儿这个事实叫 ***status***(英语 `status` 状态、地位);管它所站的那块地、那个固定位置叫 ***statio***(英语 `station` 车站、岗位)。一尊雕像,派生出三个英语词——因为它们共享同一个画面:**有东西,稳稳地,站着**。
...
大学、医院、教会、婚姻、议会——这些一听就沉甸甸的词,在词源里全是同一件事:**有人把某种做法或组织,从无到有地"立起来",让它站得住、活得久**。`institute`(动词:设立)是那个立柱子的动作,`institution`(名词:机构)是立完之后那座矗立的建筑。
...
> **提示** 这就是为什么 `existence`(存在)和 `stand`(站)、`state`(状态)同根——**古罗马人把"存在"理解为"站立"**。一个东西存在,就是它"立在那里"。
...
- **`exist` 的"站出来 = 存在"词源脉络可靠**,但"卢克莱修在庭院看嫩苗"这一具体场景是本书为助记搭的画面 *(传说)*——卢克莱修确实在《物性论》里反复讨论"存在与生成",但他没留下"盯着一株嫩苗顿悟"的记载。词是真的,人是真的,场景是搭的。
- **`substance` 译希腊哲学的 *ousia* 是学界共识**,但"哲学家指着橡树讲解实体"是通俗化的助记场景,亚里士多德本人在《范畴篇》里的论证要严密(也枯燥)得多。`substance` 与 `matter`(质料)不能画等号,前者更接近"实体/本体"。
- **斯巴达克斯起义是真的**,奴隶军"对着罗马军团站着不退"也符合史实;但用 `resistere` 这个拉丁词去命名他们的姿态,是词源助记,不是说斯巴达克斯当年喊过"我们在 resist"。
...
> **可用 `stat = 站` 串联本章词族。`state`、`exist`、`resist` 的动作画面用于助记,现代词义仍来自各自完整的历史路径。**

---

### 【思考题】(答案见附录 D)

1. `persist`(坚持)字面是"一直站着",想象一个人在风暴中站着不倒的画面。
2. `constitution`(宪法)字面是"一起立起来",为什么国家根本大法叫"立起来"?(提示:国家被"立"在它之上)
3. `substance`(本质)字面是"底下站着的",想想这个词如何体现"本质"的含义?(提示:支撑表面的根基)
