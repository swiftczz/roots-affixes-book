...
flowchart TD
    phys["物理:站(站着不动)"]
    phys --> stable["稳定(站得稳)<br/>stable, stability"]
    phys --> build["建立(使之站着)<br/>state, establish, institute"]
    phys --> state2["状态(站立的样子)<br/>status, statute"]
    phys --> pos["地位(站的位置)<br/>station, static"]
    phys --> insist["坚持(继续站着)<br/>resist, persist, insist"]
    phys --> exist["存在(站起来=出现)<br/>exist, subsist"]
    phys --> subs["物质(底下站着的)<br/>substance, substantial"]
```
...
flowchart LR
    pie["原始印欧语<br/>*stā- (站)"]
    pie --> lat["拉丁 stare (站)"]
    pie --> ger["日耳曼 *standaną"]
    pie --> grk["希腊 histanai (使站)"]
    lat --> en["state, station, exist, stable"]
    ger --> oe["古英语 standan<br/>→ stand (站)"]
    grk --> en2["system, static"]
```
