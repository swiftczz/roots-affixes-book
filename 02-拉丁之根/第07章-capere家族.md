...
flowchart TD
    root["capere<br/>(取、抓)"]
    root --> box1["基本词干及派生:cap- / capt-<br/>capere / capax / captum<br/>→ capable, capacity, capture"]
    root --> box2["复合词中的弱化形式:-cip- / -cept-<br/>recipere / conceptus / acceptus<br/>→ recipient, concept, accept, except"]
    root -->|另一条路| box3["古法语路线:ceiv-<br/>拉丁复合词 → 古法语 -ceivre / -cevoir<br/>→ 英语 -ceive<br/>例:receive, deceive, perceive, conceive"]
```
...
flowchart TD
    root["拉丁 capere<br/>(取)"]
    root --> cap["cap- / capt-<br/>(基本/分词)"]
    root --> cip["cip-<br/>(复合弱化)"]
    root --> cept["cept-<br/>(复合分词)"]
    root --> ceiv["ceiv-<br/>(法语)"]
    ...
    cept --> concept["concept(概念)"]
    cept --> accept["accept(接受)"]
    cept --> except["except(除外)"]
    cept --> intercept["intercept(拦截)"]

    ceiv --> receive["receive(接收)"]
    ceiv --> conceive["conceive(构思)"]
    ceiv --> perceive["perceive(察觉)"]
    ceiv --> deceive["deceive(欺骗)<br/>(ceive 是一组四兄弟)"]
```
...
```mermaid
flowchart LR
    obj["外界对象<br/>(多个事物)"] --> mind["心智形成构想<br/>con- + capere 词族<br/>= 在心中取入、构想 → 概念"]
```
...
```mermaid
flowchart LR
    A["能容纳(容器)<br/>capacity(容量)"] --> B["能抓住(能力)<br/>capable(能抓住)"] --> C["有能力的<br/>capable(有能力的)"]
```
...
```mermaid
flowchart LR
    world["外部世界<br/>(事物)"] -- "透过感官" --> mind["心智:感知<br/>per-(透过) + ceiv(取得)<br/>= 透过感官抓住 → 察觉"]
```
...
```mermaid
flowchart TD
    L1["capere<br/>(拉丁:取、抓)"] --> L2["captare<br/>(拉丁:试图抓住,反复抓)<br/>capere 的频率动词形式"]
    L2 -->|罗马帝国崩溃后<br/>通俗拉丁在西欧分化| branch{{"分支"}}
    branch --> R1["古法语 chacier<br/>→ 英语 chase<br/>(追逐)"]
...
> **五张脸,一个"抓"**:cap 原形(capable),capt 分词(capture),cip 复合弱化(recipient),cept 复合分词(accept),ceiv 法语路线(receive)——认出哪张脸,先想"抓"。
