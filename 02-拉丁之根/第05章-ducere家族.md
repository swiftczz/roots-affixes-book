...
**同根亲戚**:`viaduct` /ˈvaɪədəkt/(高架桥,*via* 路 + *duct* 引导——把路引导过山谷)、`aqueduct` /ˈækwəˌdʌkt/ 的近亲 *aqueous* /ˈeɪkwiəs/(水的)。`duct` 本身也是个独立词(管道、导管),所有"引导水/气/电的通道",都归这个家族管。
...
ducere 这条血脉里,还有一位**最浪漫的"引导者"**——威尼斯总督,**Doge** /doʊdʒ/。
...
每年升天节(*Festa della Sensa*,复活节后第四十天的周四),威尼斯总督登上他专属的金碧辉煌的大船——**Bucintoro(布钦托罗号)**,在舰队、贵族、神职人员的簇拥下驶出利多海峡,进入亚得里亚海的辽阔水域。船行至海中央,总督举起一枚**金戒指**,郑重地把它扔进大海,同时念出那句流传了几个世纪的誓词:
...
**同根兄弟**:`duct`(管道,引导水/气的通道)、`duchess` /ˈdʌtʃəs/(女公爵)、`duchy` /ˈdʌtʃi/(公爵领地)、`Doge`(威尼斯总督)
...
> **duc/duct = 领路**:pro 往前领(生产),re 往回领(减少),intro 领进门(介绍),con 一起领(指挥),aque 把水领过来(渡槽)——谁在前面带,词义就跟着走。

---

### 【思考题】(答案见附录 A)

1. `aqueduct` /ˈækwəˌdʌkt/(渡槽)怎么拆?为什么"引导水的"等于渡槽?
2. `seduce` /sɪˈdus/(引诱)字面义是"引导分开"——为什么引诱等于"引开"?(提示:把人从正道上引开)
3. `deduce` /dɪˈdus/(推断)字面义是"向下引导"——为什么推断等于"向下"?(提示:从普遍原则向下推导到具体)
