...
`mortgage` 由古法语 *mort*(死)+ *gage*(质押、担保)组成,字面确实是 **“死的质押”——死抵押**。但这不是说你去银行签字时,柜员会从抽屉里顺手掏出一具小棺材。

更稳妥的法史解释,要把它和 **“活抵押”**放在一起看。中世纪法律中常区分两类安排:

> - **活抵押(vivum vadium)**:土地等抵押物产生的租金、收益会逐步冲减债务,债务像在自己挣钱还自己。
> - **死抵押(mortuum vadium / mortgage)**:抵押物的收益不自动冲减本金,对债务来说像一块“不长肉、不还钱”的死担保。

这才是“死”最值得先记的地方:**死的不是哪一方,而是抵押收益对清偿本金不起“活作用”。** 一边是会慢慢啃掉债务的活抵押,一边是收着收益、债务却原地不动的死抵押——这组对照已经够阴森,不需要再给合同安排葬礼。
...
- **mortgage 先记“活抵押 vs 死抵押”的法史对照**——活抵押(vivum vadium)的收益冲减债务,死抵押(mortgage)的收益不自动冲减本金。“还清或违约总有一方死”的说法可以当助记漫画,不能反过来冒充出生证明。
...
2. **mortgage(抵押)= 死抵押**——更可靠的背景是与“活抵押”对照:活抵押的收益会冲减债务,死抵押的收益不自动冲减本金。“总有一方死”只是后来的助记漫画。
3. **murder 和 mortal 同根**——都来自原始印欧语 *mer-(死亡),走不同语言路线。

### 记忆锚点

> **mort = 死**:mortal 会死的,immortal 不死的,mortgage 是与“活抵押”相对的死抵押,mortify 治死欲望→羞愧,amortize 让债务慢慢“消失”。

---

### 【思考题】(答案见附录 A)

1. `mortgage` 常被解释成"还清了地对债权人死、违约了地对债务人死,两头都会死",很好记。但本章说这不是法史学界更认可的解释。更严谨的说法(活抵押 vs 死抵押)是什么?为什么"好记的故事"和"可靠的解释"会不一样,遇到这种情况该怎么办?
2. 本章对 `murder`/`mortal` 敢直接说"同根",对 `morbid` 却只说"可能远亲、有争议"——两个词都带 `mor-`,为什么把握程度不同?这种"证据足就下结论、不足就标存疑"的做法,体现了什么态度?
3. `mortal` 既表示"凡人的、终有一死的",又能在 `mortal wound`(致命伤)、`mortal enemy`(不共戴天的敌人)里表示"致命的/你死我活的"。用"死"这个核心义解释这两支怎么来的;再说：词根能解释这种一词多义,为什么却不能替你决定一句话里该取哪个义?
