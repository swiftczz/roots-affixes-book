...
这一章讲两个常见的拉丁法律词族。罗马法深刻影响了欧洲大陆法和教会法;英格兰普通法走的是不同制度路线,但其法律词汇仍通过拉丁语和法语吸收了大量相关词。**词汇借用不等于制度直接继承**,否则背会 `jury` 就自动通过司法考试了。
...
**`legislate`(立法)** —— 英语动词由 `legislation` 反向形成;后者追溯到拉丁 *legis* "法律的"和 *latio* "提出、带来"。可以理解为"提出法律",但现代英语中没有一个可自由使用、表示"携带"的 `-late` 后缀。
...
**`justice`(正义、司法)** —— 经法语追溯到拉丁 *justitia*,而 *justitia* 来自 *justus*(正当的、合乎法的),同属 *jus/juris* 词族。不能把现代英语 `-ice` 当作可自由表示"……的事"的后缀。

**`jury`(陪审团)** —— 来自 jurare(宣誓)。陪审团成员**先宣誓**,然后审理案件。所以 jury 字面义是"**宣誓者团体**"。
...
> **提示** 这个词源能帮助理解 `injury` 为什么既可指身体损害,也可指法律上的权利受损。但现代各义由长期使用形成,不能概括成所有古罗马人对伤害的统一哲学定义。

**`perjury`(伪证)** —— `per-`(彻底、背离)+ `jur`(宣誓)= **彻底背离宣誓**。在法庭上宣了誓还说谎,就是伪证罪。
...
`law`(法律)经古诺尔斯语 *lag/lǫg* "被放置、确立的事物/规则"进入英语——与英语 `lay`(放置)有日耳曼语亲缘。
...
| `litigation` | 拉丁 *līs/līt-*→ *litigare/litigatio* | 争讼 → 诉讼程序 |
...
1. `privilege`(特权)字面是"私人法律",想一想现代社会哪些东西是 privilege?(免税、特殊准入、世袭权利)
2. `jury`(陪审团)字面是"宣誓者",为什么陪审要先宣誓?(提示:用誓言约束按正义说话)
3. `injure`(伤害)字面是"不正义",这种理解把伤害从物理层面提升到了什么层面?(提示:道德/法律层面)
