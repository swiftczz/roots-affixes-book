...
***（助记场景）*** 黄昏时分,罗马广场中央立着一尊铜像——某位早已作古的执政官,大理石的脚踩在底座上,一立就是几百年。罗马语词里,***statua***(雕像)、***status***(位置、状态)和 ***statio***(站立处、岗位)都围绕“站立、放置”这一词族展开。不是一尊雕像生出了三个词,而是三个亲戚恰好一起站进了这幅画。

把国家想成“站起来的共同体”很适合记 `state`:旗子一插,规矩一立,大家总算不用各站各的。不过这是助记镜头,不是罗马政治哲学的十秒总结。`state` 经法语追到拉丁 *status*(位置、状况、状态),后来才一路发展出情形、身份、政体、国家等意义。词义是走着走着长大的,不是某天听见一声“起立”就集体站成了现代国家。
...
*(助记场景)* 一位古希腊哲学家——就说是亚里士多德那一派吧——把学生带到庭院里一棵老橡树前。秋深了,叶子一片片往下掉,橡子噼里啪啦砸在土里。
...
`sub-`(在下面)+ `stant`(站)给出“在下面站着”的画面。后来拉丁哲学传统用 *substantia* 等术语处理希腊 *ousia* 一类复杂概念,事情当然比“底下藏了根柱子”麻烦得多。好在这棵橡树只负责托住你的记忆,不用参加哲学答辩;真正轮到定义 *substance* 时,再把麦克风交回原文。
...
*(助记场景)* 清晨,罗马诗人兼哲学家卢克莱修在自己的庭院里踱步。他写过一部《物性论》,这里借他搭一个画面:一件事物怎样“站出来”,让人看见它存在?
...
> **助记句**:**把 exist 想成“站出来、显出身形”**,可以帮你抓住词族画面;它不是古罗马哲学界投票通过的存在定义。

这棵嫩苗是整个 stare 家族里最闪光的助记镜头。“站”这个普通动作,借前缀和历史词形一路走进了表示“存在”的词。小苗可以替你顶住记忆,可别让它独自顶住整个西方形而上学——那份加班费实在付不起。

> **提示** `existence`、`state` 属于拉丁“站立”词族,英语 `stand` 则是更早的印欧远亲,不是拉丁 *stare* 直接生出来的孩子。族谱能告诉你它们为何眉眼相似,不能替整个古罗马哲学界发表联合声明。
...
1. 本章说 `stand`、`stay` 和 stare 家族"有更早的印欧亲缘,但不是拉丁 *stare* 直接派生的借词"。请用 `stand`(本族)和 `state`(借入)说明"同源"和"直接借入"是两回事。另外,`constitution` 的 `-ution`、`institution` 的成分,能不能像现代后缀那样随意拼接?为什么?
2. `substance` 字面是"底下站着的",常译"实体/本体";但本章提醒"不能把它直接等同于 `matter`(质料)"。从"字面拆解"到"哲学术语的精确含义",中间为什么还差一大步?遇到专业词,该靠什么确定它的准确意思?
3. 本章的 `-sist` 五兄弟(insist/persist/consist/assist/resist)都是"站",靠前缀区分。给你一个没讲的 `desist` /dɪˈzɪst/(停止):`de-`(离开)+ `sist`(站)。先推字面义、说明怎么得到"停止",再把它放回五兄弟里——为什么同一个 `sist`,前缀一换意思就差这么远?
