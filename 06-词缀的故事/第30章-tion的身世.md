...
```mermaid
flowchart TD
    A["拉丁动词 educare<br/>(教育)"] -->|形成动作名词| B["educatio<br/>(教育这件事)"]
    B -->|属格 educationis| C["法语 education"]
    C --> ENG["英语 education<br/>(教育)"]
```
...
```mermaid
flowchart LR
    L1["拉丁 nationem"] --> F1["法语 nation"] --> E1["英语 nation<br/>(国家)"]
    L2["拉丁 actionem"] --> F2["法语 action"] --> E2["英语 action<br/>(行动)"]
    L3["拉丁 optionem"] --> F3["法语 option"] --> E3["英语 option<br/>(选择)"]
```
...
```mermaid
flowchart TD
    R[("动词根 act")] --> V1["act"] --> N1["action<br/>(行动)"]
    R --> V2["react"] --> N2["reaction<br/>(反应)"]
    R --> V3["interact"] --> N3["interaction<br/>(互动)"]
    R --> V4["transact"] --> N4["transaction<br/>(交易)"]
```
...
- **`-tiō` 是动作名词后缀,不是动名词,也不是目的分词**。这三者各是各的拉丁构造:`-tiō` 形成 action/state 名词,动名词尾巴是 `-nd-`(如 `agenda` 字面"待办"),目的分词是 `-tum/-tū` 那一系。别因为都"长得像名词"就混为一谈。
- **`-tion/-sion` 是很强的名词信号,但不保证一定是抽象不可数名词**。`station`(地点)、`nation`(群体)、`question`(可数的"一个问题")都能指具体、可数的事物;后缀只负责提供线索,不承包整本语法。
- **`decide → decision` 不是英语为发音方便临时删除 `d`**。`decide` 和 `decision` 分别借入了同一拉丁词族(*decidere* / *decisionem*)的不同词干,`s` 是历史词干自带的,不是现代英语抠掉的。学习时应按词族成对记:`decide/decision`、`permit/permission`、`expand/expansion`。
- **"看动词最后一个字母决定 `-tion` 还是 `-sion`"没有万能口诀**。两者分布继承自拉丁词干及后来的法语、英语音形变化,不是现场拼写规则;遇到具体词,按整词逐个学习更靠谱。
- **不要自行给任意动词追加 `-tion`**。常见动词和名词成对记忆是高效路线(`produce/production`、`create/creation`),但把 `sleep` 拍个 `-tion` 造 `sleeption` 这种事,英语会当场退件。
- **`-tion` 听起来正式,不等于一定更好**。学术写作确实常用名词化来压缩概念,但过度名词化会降低清晰度。应根据句子功能选择 `investigation` 还是 `investigate`,不能把 `-tion` 词自动视为更"高级"的学术表达。
- **拼写相同的 `-tion`,发音可能不同**。`nation` 读 /ˈneɪʃən/(`/ʃən/`,"神"),`question` 读 /ˈkwes.tʃən/(`/tʃən/`,"晨")——这是初学者最常困惑的点,记词时连同读音一起记。
