...
`-able` 的派生能力强到什么程度?这么说吧:**英语里几乎每一个动词,身后都蹲着一个被动等待的形容词,平时是隐形的;`-able` 一盖,它就显形**。`readable`、`drinkable`、`predictable`、`unbelievable`……它把动词一个个变成"能被……的"。当然,盖出来的词成不成立,还得看语义和真实用法——劳模偶尔也会盖出没人要的废件。
...
`read`(读)被盖一下,显出 `readable`(能被读的);`solve`(解决)被盖一下,显出 `solvable`(能被解决的)。核心语义是**"能被这个动作作用的"**——也就是**被动可能性**:
...
除了本职工作"能被……",`-able` 还兼了几份差,有时表达的意思离"被动可能性"挺远。下面是它的三份兼职。
...
那么,一座堡垒是怎么变成软沙发的?故事的逻辑是:`comfortare` 先从"加固"引申出"**给力量、给人支持**",再柔化成"**安慰**";`comfortable` 也就从"被强化的"一路软化为"令人安慰的、舒适的"。这是语义学里一次彻底的"软化手术"——**同一个词,一千年前站得像堵墙,今天躺得像张沙发**。
...
本章为了讲故事,把一些复杂问题简化了。下面是几条最容易踩的坑,集中提醒:

- **`-able` 更开放,不等于任何词都能加**。`-able` 比 `-ible` 更具现代能产性,但"更愿意接活"不等于"来者不拒"。
- **`-ible` 的拼写没有万能口诀**。`-ible` 多保留在历史形成的法语、拉丁来源词中,拼写应按整词逐个学习;试图只靠一条口诀解决全部单词,通常会收到几个例外寄来的投诉信。
- **大多数 `-able` 词是被动,但少数会"叛变"成主动**。`comfortable`(令人舒适的)、`agreeable`(令人愉快的)因长期使用演变成了主动义。遇到具体词,要结合用法判断,别光看后缀。
- **`-able` 也能加在名词后,但属罕见**。`fashionable`、`peaceable` 是少数派;别因此以为名词后随意加 `-able`。
- **`-ability/-ibility` 能派生性质名词,但并非每个 `-able` 词都有对应名词**。`capable → capability` 还夹着既有词干的变化,不能想当然地套公式。
