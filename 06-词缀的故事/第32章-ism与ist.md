...
第一件,也是最风光的:给**政治/哲学学说**命名。一个主张,贴上 `-ism`,就有了身份证。`capitalism`、`socialism`、`communism`、`feminism`、`liberalism`、`nationalism`、`Marxism`——这一票词把 `-ism` 推上了"主义制造机"的王座。

第二件,画风急转直下:给**歧视、偏见**命名。这是 20 世纪 `-ism` 的另一副面孔——`racism`、`sexism`、`ageism`、`ableism`。带 `-ism` 的词,在 20 世纪逐渐带上了贬义味儿,因为批判性的词最常被这样造出来:一个不公正的现象被识别出来,被命名,才能被公开反对。可以说,20 世纪的 `-ism` 是民权运动话语的副产物——它把社会里那些原本不被点名的不公,一个一个推上了被告席。

第三件,回归本职:表示**状态、行为**。这部分是 `-ism` 最老实的差事——`tourism`(旅游业)、`alcoholism`(酗酒)、`magnetism`(磁性)、`heroism`(英雄主义)。这一类里也藏着几个本章稍后要细讲的好故事。
...
如果说 `-ism` 是一种主张,那 `-ist` 就是这种主张长出了肉身。`-ist` 是 `-ism` 的"人"形——意思是"**信奉某主义的人**",或者更宽泛地,"**从事某职业的人**"。给它一个主张,它给你造一个信徒;给它一个行当,它给你造一个从业者。
...
1. **-ism 来自希腊 -ismos,经拉丁 -ismus → 法语 -isme → 英语 -ism**——一个默默干了千年杂活的希腊小后缀,在 1789 年之后被政治家们推上了"命名机器"的工位。
...
- **`-ism` 不是 1789 年才出现的**。它早在 16 世纪就随拉丁、法语借入英语(`baptism`、`martyrism` 等是它的旧账)。1789 年之后的法国大革命只是把它从"语法零件"推成"政治命名机器"的转折点,不是它的出生证。
- **`racism` 的年代辨正**。`racism` 在 20 世纪 30 年代传播显著扩大,但目前词典记录可早至 1886 年,不能写成"1930 年代首次出现"。`sexism`、`ageism` 等词的形成时间也各不相同,需要逐个核实。
- **`-ism` 后缀本身中性,不能因为 `racism`、`sexism` 带贬义就断言整个后缀发生了单向贬义化**。"It's just another -ism"带贬义,是特定修辞用法,不是后缀属性。
- **`-ism / -ist / -ize` 三件套不能机械补齐**。`theory` 就只有 `theorist`、`theorize` 而没有常用的 `theorism`。一个词族是否配齐三件套,必须核对实际存在和含义。
- **`-ism` 能产性强,不等于造出来的词都成立**。`Trumpism`、`Brexitism` 是媒体和评论员的临时造词,应区分临时修辞性用法、特定群体标签和真正词典化的词汇。`hubris` 没有人给它加 `-ism`,就是最好的反例。
- **`alcoholism` 的命名加速了酗酒从"道德问题"到"疾病"的观念转换,但没有在某一天单独完成这一转换**。观念的转变是社会、医学、法律多重力量共同推动的,一个词只是其中一环。
- **`tourist` 是 18 世纪出现的新词,但它不等于"古代没人旅行"**。在 `tourist` 出现之前,人们因贸易、朝圣、求学、外交、探险而旅行一直存在;`tourist` 标记的是一个新类别——"为了旅游本身而旅游的人"。Grand Tour 是其中一个重要文化背景,不是全体旅行史的唯一候机厅。
- **`serendipity` 不是 `-ism`**,只是因为它"有造词人和造词日期"才被借来一讲,别因此误归入 `-ism` 家族。
...
*下一章 → [第 33 章 -ly 的小史:古英语 līc"身体"如何变成副词后缀](./第33章-ly的小史.md)*
