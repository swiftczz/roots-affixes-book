...
- **`-ism` 能产性强,不等于造出来的词都站得住脚**。`Trumpism` 是媒体临时造词,`hubris` /ˈhjubrɪs/ 却没人给它加 `-ism`——后缀再勤快,也得看原料是否适配。
...
1. **(破除误解)** 为什么 `racism` 的负面意义不能证明 `-ism` 后缀本身是贬义?再举一个中性或褒义的 `-ism` 词支持你的判断。
2. **(讲证据)** `racism` /ˈreɪˌsɪzəm/ 的"最早记录年代"和"广泛流行年代"有什么区别?为什么谈一个词"什么时候出现"时,这两者必须分开?
3. **(迁移应用)** `-ism / -ist / -ize` 是一套三件套。以 `modern` 为例造出三个词,并说明各自的词性和意思;再想：这三件套是不是任何词根都能套?
